...
_SECTIONS = [
    ("基础参数", [
        ...
        ("GPU 层数  --n-gpu-layers",
         "auto",
         "由 llama-server 自动决定卸载到 GPU 显存的层数。0 = 纯 CPU；1~N = 指定卸载层数，适合显存不足时分担部分层。切换模型后数值上限自动按 block_count 更新。"),
        ("并发数  -np",
         "1",
    ...
    ("推理速度", [
        ...
        ("CPU MoE 层数  --n-cpu-moe",
         "0（不传参），范围 0 ~ 模型层数",
         "让前 N 层 MoE 权重保留在 CPU，减少显存占用。0 = 不额外保留；1~N = 指定保留层数，切换模型后数值上限自动按 block_count 更新。数值越大越省显存但速度越慢。"),
        ("跳过预热  --no-warmup",
         "关闭",
    ...
    ("采样参数", [
        ...
        ("存在惩罚  --presence-penalty",
         "0.00，范围 0.0 ~ 2.0",
         "惩罚已经出现过的 token，与出现次数无关。0.0 = 禁用；提高后会鼓励模型更换用词或话题。"),
        ("频率惩罚  --frequency-penalty",
         "0.00，范围 0.0 ~ 2.0",
         "按 token 已出现次数施加惩罚，出现越多惩罚越强。0.0 = 禁用；适合抑制高频词反复出现。"),
        ("随机种子  -s",
         "-1（随机）",
    ...
    ("聊天模板与思考 / 推理", [
        ("聊天模板文件  --chat-template-file",
         "空（使用模型内置模板）",
         "从文件读取自定义 Jinja 聊天模板。适用于模型元数据缺少模板，或需要使用特定提示词格式的场景。"),
        ("Jinja 模板  --jinja / --no-jinja",
         "开启（默认勾选）",
         "控制 llama-server 是否启用 Jinja 模板引擎。关闭后传入 --no-jinja；使用自定义聊天模板和 Tool Calling 时应保持开启。"),
        ("思考模式  -rea / --reasoning",
         "auto，选项：auto / on / off",
    ...
    ("多模态", [
        ("自动加载 mmproj  --mmproj-auto / --no-mmproj",
         "开启（默认）",
         "自动使用可用的多模态投影文件。关闭后传入 --no-mmproj；手动指定的 mmproj 文件不受此开关影响。"),
        ("视觉编码器放 GPU  --mmproj-offload / --no-mmproj-offload",
         "开启",
    ...
    ("投机解码", [
        ...
        ("最小草稿数  --spec-draft-n-min",
         "0，范围 0 ~ 64",
         "每轮至少尝试的草稿 token 数。0 表示不强制最低数量，适合保持默认策略。"),
        ("分割概率  --spec-draft-p-split",
         "0.10，范围 0.0 ~ 1.0",
         "控制草稿分支拆分概率。保持默认通常更稳，只有在调优投机解码吞吐时再调整。"),
        ("最小概率  --spec-draft-p-min",
         "0.00，范围 0.0 ~ 1.0",
         "过滤低概率草稿 token 的阈值。0.0 = 不额外过滤；调高可减少低质量草稿。"),
        ("草稿模型  -md",
         "空",
         "指定 draft 模型 GGUF 路径。使用 draft-* 投机类型时通常需要填写；留空则不传入 -md。"),
    ]),
    ("路由模式（多模型）", [
        ("模型目录  --models-dir",
         "空（关闭）",
